...
def upgrade() -> None:
    ...
    sa.Column('status', postgresql.ENUM('SUCCESS', 'PENDING', 'INTERRUPTED', 'FAILED', name='call_status'), nullable=True),
    sa.Column('failure_reason', postgresql.ENUM('ERROR', 'NO_ANSWER', 'DELAYED', 'NOT_AVAILABLE', 'CALL_TERMINATED', name='failure_reason'), nullable=True),
    sa.ForeignKeyConstraint(['contact_id'], ['contacts.id'], ),
    ...
    op.alter_column('equipments', 'name',
               existing_type=sa.VARCHAR(length=20),
               type_=sa.String(length=50),
               existing_nullable=False)
