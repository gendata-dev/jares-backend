...
async def get_groups(db_session: DbSession, page: int = 0):
    """그룹 리스트 조회"""
    if page == 0:
...
async def delete_groups(db_session: DbSession, group_in: GroupList):
    """그룹 리스트 삭제"""
...
async def create_group(db_session: DbSession, group_in: GroupCreate):
    """그룹 생성"""
    try:
        ...
        raise HTTPException(
            ...
            detail=f"{group_in.groupName}은 이미 사용중인 이름입니다",
        )
...
async def get_group(db_session: DbSession, group_id: PrimaryKey):
    """그룹 조회"""
...
async def update_group(
    db_session: DbSession, group_id: PrimaryKey, group_in: GroupCreate
):
    """그룹 수정"""
...
async def update_group_relation(db_session: DbSession, group_id: int):
    """그룹 리스트에 연락처 리스트 추가"""
